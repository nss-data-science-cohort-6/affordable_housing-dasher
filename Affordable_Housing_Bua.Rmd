...
```{r}
#my attempt to create an interactive map on leaflet
library(leaflet)
library(raster)

county.lines.sf <- st_as_sf(county.lines, coords = c("long", "lat"))


mapStates = map("state", fill = TRUE, plot = FALSE)
leaflet(data = county.lines.sf) %>% addTiles()
  #addProviderTiles(providers$SThunderforest.SpinalMap) %>%
...
property_details_filtered %>%
  group_by(group) %>%
  summarize(meanTpost = mean(Tpost, na.rm=T))

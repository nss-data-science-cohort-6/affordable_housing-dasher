...
```{r}
setwd("/Users/thidathornvanitsthian/Documents/Data Science 2022/affordable_housing-dasher/Data")
...
3. Filter the homes down to those that are within one mile of an affordable housing development. (1 mile = 1609.34 m)

```{r}
property_within_1mile <- subset(property_details, NNDist<1609.34)
...
```{r}
property_details_merge <- merge(property_within_1mile, unique(st_drop_geometry(LIHTC_filtered)[c("HUD_ID","YR_PIS")]), by = "HUD_ID", all.x=T)
...
```{r}
property_details_filtered$age <- as.numeric(year(property_details_filtered$ownerdate) 
                                            - property_details_filtered$year_built)
...
colSums(is.na(property_details_filtered_within5yr))
```
...
```{r}
log_model <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group) + factor(tract), data = property_details_filtered_within5yr)
summary(log_model)
```

```{r}
log_model_notract <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group) , data = property_details_filtered_within5yr)
summary(log_model_notract)
anova(log_model_notract, log_model) #we need tract
```

```{r}
log_model_NNDist <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group) + factor(tract) + NNDist, data = property_details_filtered_within5yr)
summary(log_model_NNDist)
anova(log_model, log_model_NNDist)
```

Log Model with Building Condition

```{r}
log_model_bdcnd <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group) + factor(tract) + factor(building_condition), data = property_details_filtered_within5yr)
summary(log_model_bdcnd)
anova(log_model, log_model_bdcnd)
```

Plot of Log Model with Building Condition

```{r}
log_model_bdcnd_df <- data.frame(actual = log(property_details_filtered_within5yr$amount), 
                                        predicted = log_model_bdcnd$fitted.values)
log_model_bdcnd_df %>%
  ggplot() +
  geom_hex(aes(x=actual, y=predicted), bins = 100) +
  scale_fill_gradientn(colours = rainbow(100)[75:1])
```

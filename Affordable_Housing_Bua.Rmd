...
property_details_merge <- merge(property_details, unique(st_drop_geometry(LIHTC)[c("HUD_ID","YR_PIS")]), by = "HUD_ID", all.x=T)

property_details_merge <- merge(property_details_merge, unique(filtered_sales[c("apn","ownerdate","amount")]), by = "apn", all.x=T)

property_details_merge <- merge(property_details_merge, unique(assessments[c("apn","class")]), by = "apn", all.x=T)
...
unique(property_details_merge$class)

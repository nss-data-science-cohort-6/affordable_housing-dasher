...
try(st_nearest_feature(property_details, LIHTC_filtered))
...
#filter out YR_PIS >= 2000 in LIHTC first before calculating NNDist
...
#How can a home be sold before it was built? Perhaps it was the property before the current house was built
...
property_details_filtered_within5yr <- property_details_filtered %>%
  subset((YR_PIS - as.numeric(year(ownerdate))) %in% -5:5)
...
linear_model <- lm(amount ~ square_footage + age + year(ownerdate) + factor(group) + factor(tract)-1, data = property_details_filtered_within5yr)
summary(linear_model)

#-1 in factor(tract)-1 removes the global intercept
```
...
# How can you interpret the coefficients of this model?
#   
log_linear_model <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group) + factor(tract)-1, data = property_details_filtered_within5yr)
summary(log_linear_model)
```

```{r}
log_linear_model_notract <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group)-1 , data = property_details_filtered_within5yr)
summary(log_linear_model_notract)
```

```{r}
log_linear_model_NNDist <- lm(log(amount) ~ square_footage + age + year(ownerdate) + factor(group)-1 + NNDist, data = property_details_filtered_within5yr)
